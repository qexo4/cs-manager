...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // W przeciwieństwie do zwykłego dodania sesji (które sumuje zysk i dopisuje
    // wynik po przecinku), edycja NADPISUJE rekord po jego ID - żeby można było
    // poprawić literówkę w nazwie / zły link / błędnie wpisaną kwotę bez
    // zaburzania historii wyników i skumulowanego zysku innych kont.
    if (($_POST['form_action'] ?? '') === 'update') {
        ...
        // Bana ustawiamy od nowa tylko jeśli edytujący go włączył/zmienił,
        // w przeciwnym razie zerujemy (brak blokady po edycji na 0 dni).
        ...
        $msg = "✏️ **Edytowano konto w panelu!**\n";
        $msg .= "👤 **Nazwa:** $name\n";
        $msg .= "💰 **Nowy zysk skumulowany:** $profit PLN";
    ...
    // ====== ISTNIEJĄCA GAŁĄŹ: DODANIE NOWEJ SESJI ======
    $name = trim($_POST['name']);
    $steam_url = trim($_POST['steam_url'] ?? '');
    $amount = floatval($_POST['amount']);
    $end_amount = floatval($_POST['end_amount']);
